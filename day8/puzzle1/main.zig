...
fn glt(context: void, a: Group, b: Group) bool {
    _ = context;
...
pub fn main() !void {
    ...
    const NBR_OF_CONNECTIONS: usize = 1000;
    for (0..NBR_OF_CONNECTIONS) |c| {
    ...
    var groups = std.ArrayList(Group).init(gpa.allocator());
    defer groups.deinit();
    ...
    for (0..line_count) |i| {
        ...
        if (found) continue;
        try groups.append(Group{ .n = f, .count = 1 });
    }

    std.sort.block(Group, groups.items, {}, glt);
